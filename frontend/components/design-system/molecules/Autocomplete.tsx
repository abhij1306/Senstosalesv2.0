        ...
        <div ref={wrapperRef} className={cn("relative w-full", className)}>
            <div className="relative">
                <input
                    ...
                    className={cn(
                        "flex h-10 w-full rounded-xl border border-transparent bg-surface-variant/30 px-3 pl-10 py-2 text-sm text-text-primary outline-none ring-offset-background placeholder:text-text-tertiary focus:bg-surface focus:ring-2 focus:ring-action-primary/20 disabled:cursor-not-allowed disabled:opacity-50 transition-all shadow-sm",
                ...
                {!loading && query && (
                    <button
                        onClick={handleClear}
                        className="absolute right-3 top-2.5 hover:bg-surface-variant/50 rounded-full p-0.5 transition-colors"
                        type="button"
                    >
            ...
            {isOpen && (
                <div className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-xl border border-white/10 bg-surface/90 p-1 shadow-premium backdrop-blur-md animate-in fade-in zoom-in-95 duration-100">
                    ...
                    {options.map((option) => (
                        ...
                        >
                            <div className="flex flex-col gap-0.5">
                                ...
                                {option.subLabel && (
                                    <span className="text-[10px] text-text-tertiary uppercase tracking-wider">{option.subLabel}</span>
                                )}
                            </div>
                            {value === option.value && (
