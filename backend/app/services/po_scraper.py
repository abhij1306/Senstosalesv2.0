            ...
            for r_idx, row in enumerate(rows):
                ...
                for c_idx, cell in enumerate(cells):
                    ...
                    if found_val:
                        ...
                        if (found_val.upper().startswith("DETAILS") or 
                            "RAISED ON PO" in found_val.upper() or 
                            len(found_val) > 100): 
...
def extract_items(soup):
        ...
        if len(cols) >= 8 and cols[0] and cols[0].isdigit():
            ...
            if item_id is not None:
                ...
                delivery_rows.append({
                    ...
                    "RCD QTY": to_float(cols[6]) if len(cols) > 6 else None,  # Maps to received_quantity
    ...
    for item_no, item in items_map.items():
        # Handle DRG
